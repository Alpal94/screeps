...
module.exports.loop = function () {
	...
	//mainFunction(Game.spawns.Spawn1, true);
...
function mainFunction(TheTerminator, home, newBase) {
	...
	//else if(true) name = TheTerminator.createCustomCreep(energy,  'claimer', 0);
	...
	else if(numberOfLongDistanceHarvesters < 2) name = TheTerminator.createCustomCreep(energyLong,  'long_distance_harvester', 3);
	...
	*/
	...
	/*else if(numberOfHarvesters < minimumNumberOfHarvesters) {
		//name = Game.spawns.TheTerminator.createCreep([WORK, WORK, CARRY, MOVE], undefined, { role:'harvester', working:false});
		name = TheTerminator.createCustomCreep(energy,  'harvester', 0);
		
	} 
	else if( numberOfUpgraders < minimumNumberOfUpgraders) {
		//name = Game.spawns.TheTerminator.createCreep([WORK, CARRY, MOVE, MOVE], undefined, { role:'upgrader', working:false});
		name = TheTerminator.createCustomCreep(energy,  'upgrader', 0);
	}
	else if( numberOfRepairers < minimumNumberOfRepairers) {
		//name = Game.spawns.TheTerminator.createCreep([WORK, WORK, CARRY, MOVE], undefined, { role:'repairer', working:false});
		name = TheTerminator.createCustomCreep(energy,  'repairer', 0);

	}
	else if( numberOfBuilders < minimumNumberOfBuilders) {
		//name = Game.spawns.TheTerminator.createCreep([WORK, WORK, CARRY, MOVE], undefined, { role:'builder', working:false});
		name = TheTerminator.createCustomCreep(energy,  'builder', 0);
	}
	else if( numberOfWallRepairers < minimumNumberOfWallRepairers) {
		//name = TheTerminator.createCreep([WORK, WORK, CARRY, MOVE], undefined, { role:'builder', working:false});
		name = TheTerminator.createCustomCreep(energy,  'wall_repairer', 0);
		console.log("Spawned wall repairer");
	}*/
